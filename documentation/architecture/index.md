...
!!! tip "ACK Legend"
    ...
    - ack(received): Router durably accepted and persisted the message; origin records receipt and retries if this is not observed before timeout.
    - ack(processed): Target handler completed successfully; origin finalizes lifecycle and no further retries will be issued.
...
- **Explicit ACK lifecycle**: Delivery confirmation is explicit (received → processed) with idempotency keys, bounded retries with exponential backoff, and DLQ on exhaustion. This yields at-least-once delivery while preventing duplicate side effects in handlers via idempotent processing.
...
sequenceDiagram
    ...
    TGT-->>RTR: ack(processed)
    RTR-->>SDK: ack(processed)
    SDK->>AB: finalize message state
```

!!! tip "ACK Legend"

    - ack(received): Router durably accepted and persisted the message at ingress to the Router (not shown in this inbound diagram).
    - ack(processed): Emitted by the target after successful handling; Router relays to origin which then finalizes lifecycle state.


Explanation


- send(message): The application calls the SDK, which assigns a `message_id` and `idempotency_key`, stamps causal metadata (e.g., vector clock), and prepares delivery options (TTL, priority, retries).
...
- ack(processed): After the target handler completes successfully (side effects committed), the target SDK emits ack(processed) back via the Router.
...
- Semantics: ack(received) = durably accepted by Router; ack(processed) = successfully handled by the target. This supports at‑least‑once delivery with idempotent handlers.
...
- Dedup/idempotency: The idempotency key (plus causal metadata) prevents re-applying side effects when redeliveries occur due to retries or network partitions.
...
- Acknowledgment: The SDK sends ack(processed) only after successful handler completion and post-hooks, preserving at-least-once semantics.
...
- **ACKLifecycleManager**: Tracks message state transitions (received → processed) with idempotency keys and emits retries using exponential backoff with jitter and caps. It deduplicates redeliveries, records DLQ handoff on exhaustion, and exposes per-message outcome metrics for observability.
...
- **Activity Buffer**: Uses atomic, append-only writes with periodic compaction and retention policies to bound disk usage. Recovery reprocesses only incomplete lifecycles and preserves idempotency by resending the same `message_id` and `idempotency_key`.
...
- **Delivery semantics**: Implements at-least-once delivery with bounded retries, exponential backoff with jitter, and DLQ on exhaustion, while preserving the same idempotency key across attempts. Timeouts are enforced with monotonic clocks and traced so operators can correlate redeliveries with upstream failures.
