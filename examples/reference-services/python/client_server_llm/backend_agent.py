...
GEN_ROOT = ROOT / "generated_app" / "backend"
...
_COLOR_RESET = "\033[0m"
_COLOR = "\033[36m"  # cyan for backend
_GREEN = "\033[32m"
_RED = "\033[31m"

def _tag() -> str:
    return f"{_COLOR}[backend]{_COLOR_RESET}" if _supports_color() else "[backend]"
...
_SESSION_FILE = ROOT / ".backend_session_id"
...
def write_backend_files(result_obj: dict, fallback_prompt: str) -> list[str]:
    ...
    if isinstance(files, list):
        ...
        def _normalize_rel_path(rel: str) -> pathlib.Path:
            ...
            # Strip leading generated_app and backend if present
...
def run_claude(prompt: str) -> dict:
    """Use Claude CLI (stream-json) to obtain JSON response."""
...
def main() -> int:
    ...
    def _hb():
        while True:
            try:
                reg.heartbeat(agent_id=agent_id, state=0, health={"role": "backend"})
            except Exception:
                pass
            time.sleep(30)
    _threading.Thread(target=_hb, daemon=True).start()
    ...
    for item in router.stream_incoming(agent_id):
        ...
        if stage == "generate":
            ...
            files_written = write_backend_files(result_obj, prompt)
