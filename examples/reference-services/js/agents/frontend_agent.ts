...
// Frontend agent for the client_server_llm demo (JS)
// Registers as "frontend", listens for CONTROL scheduler commands (generate/run),
// invokes `claude` CLI, writes files under ./generated_app/frontend, and serves frontend on run.
...
const GEN_ROOT = path.join(ROOT, 'generated_app', 'frontend');
const SESSION_FILE = path.join(ROOT, '.frontend_session_id');

// --- logging helpers ---
const COLOR_RESET = '\u001b[0m';
const COLOR = '\u001b[35m'; // magenta for frontend
const GREEN = '\u001b[32m';
const RED = '\u001b[31m';
function supportsColor(): boolean {
  try { return process.stdout.isTTY && !process.env.NO_COLOR; } catch { return false; }
}
function tag(): string { return supportsColor() ? `${COLOR}[frontend]${COLOR_RESET}` : '[frontend]'; }
...
function writeFrontendFiles(resultObj: any, fallbackPrompt: string): string[] {
  fs.mkdirSync(GEN_ROOT, { recursive: true });
  ...
  if (files) {
    for (const f of files) {
      try {
        ...
        if (parts[0] === 'frontend') parts = parts.slice(1);
  ...
  // Minimal fallback app (Python-parity)
...
<body>
  ...
  const indexJs = `const qs = new URLSearchParams(location.search);
...
document.getElementById('btn').addEventListener('click', async () => {
  ...
  fs.writeFileSync(path.join(GEN_ROOT, 'index.html'), indexHtml, 'utf8');
  fs.writeFileSync(path.join(GEN_ROOT, 'index.js'), indexJs, 'utf8');
  fs.writeFileSync(path.join(GEN_ROOT, 'PROMPT.txt'), fallbackPrompt ?? '', 'utf8');
  return ['index.html', 'index.js', 'PROMPT.txt'];
}
...
async function main() {
  ...
  // Heartbeats
  setInterval(() => {
    registry.heartbeat(agentId, 'AGENT_STATE_UNSPECIFIED', { role: 'frontend' }).catch(() => {});
  }, 30_000);
  ...
  stream.on('data', async (item: any) => {
    ...
    if (stage === 'generate') {
      ...
      const filesWritten = writeFrontendFiles(resultObj, prompt);
