...
const WORKTREE_ADDR = process.env.SW4RM_WORKTREE_ADDR
  || `${process.env.WORKTREE_HOST || 'localhost'}:${process.env.WORKTREE_PORT || '50055'}`;

async function main() {
  const worktree = new WorktreeClient({ address: WORKTREE_ADDR });
