...
type ReasoningProxyClient = grpc.Client & {
  ...
  Summarize(req: { session_id: string; segments: Array<{ kind: string; content: string; seq: number | string; at: string }>; max_tokens?: number; mode?: string }, meta: grpc.Metadata, opts: grpc.CallOptions, cb: (err: grpc.ServiceError | null, res: { summary: string; tokens: number; cost_cents: number; model: string }) => void): void;
};
...
export class ReasoningClient extends BaseClient {
  ...
  summarize(sessionId: string, segments: Array<{ kind: string; content: string; seq: number; at: string }>, maxTokens?: number, mode?: string): Promise<{ summary: string; tokens: number; cost_cents: number; model: string }> {
    const meta = this.metadata();
    const opts: grpc.CallOptions = { deadline: this.deadlineFromNow() };
    return this.withRetryUnary(() => new Promise((resolve, reject) => {
