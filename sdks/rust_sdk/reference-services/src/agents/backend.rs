...
fn gen_root() -> PathBuf { demo_root().join("generated_app").join("backend") }
fn session_file() -> PathBuf { demo_root().join(".backend_session_id") }
...
fn normalize_rel(rel: &str) -> PathBuf {
    ...
    if parts.first() == Some(&"backend") { parts.remove(0); }
...
fn write_backend_files(result_obj: &Value, fallback_prompt: &str) -> Vec<String> {
    ...
    // Fallback minimal server.py with CORS
...
"##;
    let _ = fs::write(gen_root().join("server.py"), server_py);
    let _ = fs::write(gen_root().join("PROMPT.txt"), fallback_prompt);
    vec!["server.py".into(), "PROMPT.txt".into()]
}
...
pub async fn run() -> Result<()> {
    ...
    let agent_id = "backend".to_string();
    let agent = AgentDescriptor::new(agent_id.clone(), "Backend Agent".into())
        .with_description("Generates backend server".into())
    ...
    tokio::spawn(async move {
        loop {
            let _ = hb_registry.heartbeat(&aid, AgentState::Unspecified, None).await;
    ...
    while let Some(item) = tokio_stream::StreamExt::next(&mut stream).await {
        ...
        match stage {
            "generate" => {
                ...
                let files_written = write_backend_files(&result_obj, prompt);
                let report = json!({
                    "schema_version": 1,
                    "stage": "generate",
                    "status": "ok",
