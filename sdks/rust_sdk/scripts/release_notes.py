    ...
    while i < len(lines):
        if "\x00" in lines[i]:
            ...
            while i < len(lines) and lines[i] == "":
                i += 1
            if any(p.startswith("sdks/rust_sdk/") for p in touched):
...
def main():
    ...
    print(f"# Rust SDK {tag.removeprefix(TAG_PREFIX)} Release Notes\n")
    if since:
    ...
    if not commits:
        print("No changes scoped to sdks/rust_sdk in this range.")
