...
use crate::proto::sw4rm::activity::{AppendArtifactRequest, Artifact, ListArtifactsRequest};
...
impl ActivityClient {
    pub async fn new(endpoint: &str) -> Result<Self> {
        let channel = Endpoint::from_shared(endpoint.to_string())
            ...
            .await?;
        Ok(Self {
            client: ActivityServiceClient::new(channel),
        })
    }

    pub async fn append_artifact(&mut self, artifact: Artifact) -> Result<bool> {
        let req = AppendArtifactRequest {
            artifact: Some(artifact),
        };
        let res = self
            .client
            .append_artifact(tonic::Request::new(req))
            .await?
            .into_inner();
        Ok(res.ok)
    }
    ...
    pub async fn list_artifacts(
        ...
        let res = self
            .client
            .list_artifacts(tonic::Request::new(req))
            .await?
            .into_inner();
        Ok(res.items)
    }
