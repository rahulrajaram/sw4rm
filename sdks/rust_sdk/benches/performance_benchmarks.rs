...
fn benchmark_activity_buffer(c: &mut Criterion) {
    ...
    for capacity in buffer_sizes {
        ...
        let test_messages: Vec<_> = (0..100).map(|i| {
            json!({
    ...
    group.bench_function("agent_message_processing", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            impl Agent for BenchmarkAgent {
                async fn on_startup(&mut self) -> Result<()> { Ok(()) }
                ...
                async fn on_control(&mut self, _envelope: EnvelopeData) -> Result<()> { Ok(()) }
                ...
                fn preemption_manager(&self) -> &PreemptionManager {
                    static PREEMPTION: std::sync::OnceLock<PreemptionManager> = std::sync::OnceLock::new();
...
fn benchmark_memory_usage(c: &mut Criterion) {
    ...
    group.bench_function("envelope_memory_allocation", |b| {
        b.iter(|| {
            let payloads: Vec<_> = (0..1000).map(|i| {
                EnvelopeBuilder::new(
                    format!("agent-{}", i),
                    constants::message_type::DATA
                )
