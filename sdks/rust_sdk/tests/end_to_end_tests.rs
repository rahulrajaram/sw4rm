...
async fn test_activity_buffer_ack_lifecycle() {
    ...
    for (i, msg) in messages.iter().enumerate() {
        ...
        if i < 2 {
            ...
            assert_eq!(record.error_code, constants::error_code::UNSUPPORTED_MESSAGE_TYPE);
        }
    }
...
async fn test_message_processor_ack_integration() {
    ...
    let record = buffer.record_incoming(serde_json::to_value(&envelope).unwrap()).unwrap();
    ...
    assert_eq!(ack_envelope.message_type, constants::message_type::ACKNOWLEDGEMENT);
...
async fn test_agent_lifecycle_simulation() {
    ...
    impl Agent for TestAgent {
        ...
        async fn on_shutdown(&mut self) -> Result<()> {
            println!("Test agent shutting down: {} ({} messages processed)", 
    ...
    let control_envelope = EnvelopeBuilder::new("controller".to_string(), constants::message_type::CONTROL)
...
async fn test_worktree_integration() {
    let temp_file = NamedTempFile::new().unwrap();
    let persistence = Box::new(sw4rm_sdk::worktree_state::JsonWorktreePersistence::new(temp_file.path()));
    let policy = Box::new(sw4rm_sdk::worktree_state::DefaultWorktreePolicy::new());

    let mut worktree_state = sw4rm_sdk::worktree_state::PersistentWorktreeState::new(
        Some(persistence), 
        Some(policy)
    ).unwrap();
    ...
    let new_persistence = Box::new(sw4rm_sdk::worktree_state::JsonWorktreePersistence::new(temp_file.path()));
