...
def push_tags(tags):
    ...
    print(f"Pushed tags: {' '.join(tags)}")


def main():
    ap = argparse.ArgumentParser(description="Create release tags for all SDKs")
    ap.add_argument("version", help="SemVer X.Y.Z")
    ap.add_argument("--push", action="store_true", help="Push the created tags to origin")
    args = ap.parse_args()
